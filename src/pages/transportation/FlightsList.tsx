...
const FlightsList = () => {
  ...
  const { 
    onwardItems: onwardFlights, 
    returnItems: returnFlights,
    loading, 
    error, 
    filters, 
    updateFilters, 
    ...
    handleInitialSearch,
    initialSearchDone
  } = useTransportationSearch('flight');
  ...
  useEffect(() => {
    ...
    if (!initialSearchDone) {
      ...
      // If we just have fromCity and toCity (direct navigation to flights page)
  ...
  useSeo({
    title: `Flights from ${filters.fromCityName} to ${filters.toCityName} | TravelEase`,
    description: `Find and book flights from ${filters.fromCityName} to ${filters.toCityName} with TravelEase.`,
  });
      ...
      <div className="flex-1 mt-24">
        <div className="container-custom py-8">
          <div className="flex items-center gap-3 mb-6">
            <Plane size={24} />
            <h1 className="text-3xl font-display font-bold">Flights</h1>
          </div>
            ...
            <div className="mb-8">
              ...
              <div className="mb-8">
                <h2 className="text-xl font-semibold mb-4">
                  {filters.tripType === 'round-trip' ? 'Outbound Flights' : 'Available Flights'}
                </h2>
                <p className="text-gray-600 mb-6">
                  {onwardFlights.length > 0 ? (
                    `Found ${onwardFlights.length} flights from ${filters.fromCityName} to ${filters.toCityName}`
                  ) : (
                ...
                {onwardFlights.length > 0 ? (
                  onwardFlights.map((flight) => (
                    <TransportationCardV2 
                      key={flight.TransportationID} 
                      transportation={flight} 
                    />
                  ))
                ) : (
                  <div className="text-center py-12 bg-gray-50 rounded-md">
                    <h3 className="text-xl font-medium text-gray-600 mb-2">No flights available</h3>
              ...
              {/* Return Flights Section - Only show if tripType is round-trip and we have return flights */}
              {filters.tripType === 'round-trip' && returnFlights.length > 0 && (
                <div>
                  <h2 className="text-xl font-semibold mb-4">Return Flights</h2>
                  <p className="text-gray-600 mb-6">
                    Found {returnFlights.length} flights from {filters.toCityName} to {filters.fromCityName}
                  </p>
